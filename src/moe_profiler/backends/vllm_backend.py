"""vLLM subprocess backend with streamed OpenAI-compatible measurements."""
...
class VllmBackend(Backend):
    """Manage a vLLM server and measure streamed generation latency."""
    ...
    def version(self) -> str:
        """Return package metadata without importing vLLM."""
        try:
    ...
    def start(
        ...
        **kwargs: object,
        ...
        """Launch ``vllm serve`` and poll its health endpoint."""
        if self._process is not None:
            raise BackendError("vLLM backend is already running")

        executable = shutil.which("vllm")
        if executable is None:
            raise BackendUnavailableError(
                "vLLM backend unavailable: the 'vllm' command is not installed "
                "in this environment. Install vLLM in a dedicated GPU environment."
            )
        ...
        try:
            ...
            raise BackendUnavailableError(f"Could not launch vLLM: {exc}") from exc
    ...
    def generate(self, requests: list[RequestSpec]) -> list[GenerationResult]:
        ...
        if self._process is None or self._process.poll() is not None:
            raise BackendError("vLLM backend is not running")
        if self._model_id is None:
            raise BackendError("vLLM backend has no configured model")
        ...
        with httpx.Client(timeout=2.0) as client:
            while time.monotonic() < deadline:
                ...
                if exit_code is not None:
                    raise BackendUnavailableError(
                        f"vLLM server exited during startup with code {exit_code}"
                    )
                try:
                ...
                time.sleep(0.5)
        raise BackendUnavailableError(
            f"vLLM did not become healthy within {self.startup_timeout_s:.1f}s"
        )
